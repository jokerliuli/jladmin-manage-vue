<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input
        v-model.trim="picture.pictureTitle"
        clearable
        placeholder="图片标题"
        style="width: 200px;"
        class="filter-item"/>
      <el-select v-model="picture.pictureType" placeholder="图片分类" clearable class="filter-item" style="width: 110px">
        <el-option
    ...
    >
      ...
      <el-table-column prop="pictureType" label="图片分类"/>
      <el-table-column prop="sort" label="排序"/>
      <el-table-column label="关联成功案例">
        <template slot-scope="scope">
          <el-button type="text" @click="handleSuccessUpdate(scope.row.successId)">{{ scope.row.successId }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="mini"
            type="success"
            @click="handleModifyStatus(scope.row, 2)">发布
          </el-button>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="textMap[dialogStatus]" width="35%">
      <el-form
        ...
        <el-form-item label="图片分类" prop="pictureType">
          <el-select v-model="temp.pictureType" placeholder="图片分类" clearable class="filter-item" style="width: 110px">
            <el-option
              v-for="item in pictureTypeOptions"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="成功案例" prop="successId">
          <el-input v-model.number="temp.successId"/>
        </el-form-item>
...
import { remove, page, update, save } from '@/api/achievements'
import { count } from '@/api/success'
...
export default {
  ...
  data() {
    return {
      pictureTypeOptions: [
        // 新闻动态，产品方案，成功案例
        { value: '工业互联网', label: '工业互联网' },
        { value: '智慧农业', label: '智慧农业' },
        { value: '智慧公安', label: '智慧公安' },
        { value: '智慧环保', label: '智慧环保' },
        { value: '智慧交通', label: '智慧交通' },
        { value: '智慧教育', label: '智慧教育' },
        { value: '智慧养老', label: '智慧养老' }
      ],
      ...
      rules: {
        ...
        pictureDescription: [
          { required: true, message: '请填写图片描述(500字符以内)', trigger: 'blur' },
        ...
        successId: [
          { required: false, message: '请填写绑定成功案例的id', trigger: 'blur' },
  ...
  methods: {
    handleSuccessUpdate(id) {
      count({ id: id }).then(response => {
        const result = response.data.data
        if (result > 0) {
          this.$router.push({ path: '/information/EditSuccess', query: { id: id }})
        } else {
          this.$message({
            message: '不存在该id的成功案例,请修改为正确id',
            type: 'error'
          })
        }
      })
    ...
    open5(url) {
      this.$alert('<img width="100%" src=' + url + ' />', '预览', {
    ...
    getListBypage() {
      ...
      page(this.listQuery, this.picture).then(response => {
        this.picturesList = response.data.data.records
